...
cols<-brewer.pal(6,"Paired")
pdf("../../project_results/figures/pca_clean.pdf",width=12,height=8)
pca<-princomp(df[1:6])
plot(pca$loading,pch=19, cex=2,col=cols)
text(pca$loading, names(geneResults),pos = 1)
dev.off()

pdf("../../project_results/figures/pca_components_clean.pdf",width=12,height=8)
plot(pca)
dev.off()
...
pdf("../../project_results/figures/logFC_genes_25plus25minus.pdf",width=12,height=8)
plot(pca)
#dev.off()
...
pdf("../project_results/endo_1.figures/normalized_pca2.pdf",width=12,height=8)
