...
dge_genes <- row.names(y$counts)[decideTestsDGE(lrt) == 1]


egSYMBOL <- toTable(org.Mm.egSYMBOL)


dplyr::filter(egSYMBOL, gene_id %in% dge_genes)
...
save.image(paste0(projectDir, "/Robjects/", samplename, "_edgeR_from_rsemCounts_temp_quek.RData"))
